...
class GameStateManager:
    ...
    def get_game_state(self, game_state_id: int):
        ...
        phase = execute(Query(query, (game_state_id,)))[0]
    ...
    def setup_voting(self, game_state_id: int):
        if not self.has_game_state(game_state_id):
            raise ValueError(f"No game state with id {game_state_id}.")
        query = f"SELECT parameter FROM controls WHERE game_state = %s AND buergerrat = 1;"
        all_parameters_br1 = [row[0] for row in execute(Query(query, (game_state_id,)))]
        query = f"SELECT parameter FROM controls WHERE game_state = %s AND buergerrat = 2;"
        all_parameters_br2 = [row[0] for row in execute(Query(query, (game_state_id,)))]
        query = f"SELECT session_id FROM Session WHERE game_state = %s;"
        session_id = execute(Query(query, (game_state_id,)))[0][0]
        ...
        for users, all_parameters in [(users_br1, all_parameters_br1), (users_br2, all_parameters_br2)]:
            for username in users:
                for parameter in all_parameters:
                    query = f"INSERT INTO Voting(user, parameter) VALUES (%s, %s);"
    ...
    def voting_have_all_voted(self, game_state_id: int) -> bool:
        ...
        query = (f"SELECT COUNT(*) "
                 ...
                 f"WHERE Session.game_state = %s AND Voting.voted_value IS NULL;")
        n = execute(Query(query, (game_state_id,)))[0][0]
        return n == 0
    ...
    def voting_has_voted(self, username: str, parameter: str) -> bool:
        ...
        query = f"SELECT COUNT(*) FROM Voting WHERE user = %s AND parameter = %s AND voted_value IS NOT NULL;"
        n = execute(Query(query, (username, parameter)))[0][0]
        return n > 0
